...
document.getElementById('saleSearch')?.addEventListener('keyup', function(e) {
    ...
    document.querySelectorAll('.sale-row').forEach(function(row) {
        var customerName = row.querySelector('td:nth-child(2)').textContent.toLowerCase();
        var phoneNumber = row.querySelector('td:nth-child(3)').textContent.toLowerCase();
        var matches = customerName.includes(searchText) || phoneNumber.includes(searchText);
    ...
    if (visibleCount === 0 && document.querySelectorAll('.sale-row').length > 0) {
        if (!noResultsRow) {
            ...
            noResultsRow.innerHTML = '<td colspan="7" class="text-center py-4"><i class="bi bi-inbox fs-1 text-muted"></i><p class="text-muted mt-2">কোনো মিল খুঁজে পাওয়া যায়নি</p></td>';
